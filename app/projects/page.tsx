...
const projectNames = {
  en: ['Pulse Banking', 'Bloom Marketplace', 'Atlas CRM', 'Nomad Booking', 'Volt Fitness', 'Medix Telehealth'],
  ar: ['بولس المصرفي', 'سوق بلوم', 'أطلس CRM', 'نوماد للحجوزات', 'فولت للياقة', 'ميديكس للطب عن بعد'],
}
          ...
          <div style={{ display: 'flex', gap: 'var(--space-2)', justifyContent: 'center', flexWrap: 'wrap' }}>
            {industries.map((industry) => (
              <button
                ...
                style={{ boxShadow: selectedIndustry === industry.key ? 'var(--shadow-brutal)' : 'none' }}
              >
                {industry.label}
              </button>
            ))}
      ...
      <section className="section" style={{ paddingTop: 'var(--space-8)', paddingBottom: 'var(--space-16)' }}>
        <div className="container">
          <div className="stats" style={{ maxWidth: '900px', margin: '0 auto' }}>
            <div className="stat">
              <div className="stat-value">{t.projectsPage.stats.projects.value}</div>
              <div className="stat-label">{t.projectsPage.stats.projects.label}</div>
            </div>
            <div className="stat">
              <div className="stat-value">{t.projectsPage.stats.timeline.value}</div>
              <div className="stat-label">{t.projectsPage.stats.timeline.label}</div>
            </div>
            <div className="stat">
              <div className="stat-value">{t.projectsPage.stats.industries.value}</div>
              <div className="stat-label">{t.projectsPage.stats.industries.label}</div>
            </div>
            <div className="stat">
              <div className="stat-value">{t.projectsPage.stats.satisfaction.value}</div>
              <div className="stat-label">{t.projectsPage.stats.satisfaction.label}</div>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container">
            ...
            {filteredProjects.map((project) => (
              ...
              >
                ...
                <div className="project-body">
                  ...
                  <h3 className="project-title">{projectNames[lang as 'en' | 'ar'][project.titleKey]}</h3>
                  <p className="project-description">{projectDescriptions[lang as 'en' | 'ar'][project.titleKey]}</p>
          ...
          {filteredProjects.length === 0 && (
            <div style={{ textAlign: 'center', padding: 'var(--space-16) 0' }}>
              <p style={{ color: 'var(--fg-muted)', fontSize: 'var(--text-lg)' }}>{t.projectsPage.noProjects}</p>
              <button
